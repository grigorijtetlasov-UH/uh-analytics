...
# Загальні набори статусів — single source of truth.
# Узгоджено з sales_kpi.py (еталон власника, 19.05.2026).
# Якщо змінюєш список — синхронізуй з sales_kpi.py (і навпаки).
_ORDER_STATUSES = {
...
_LOST_LEAD_STATUSES = {
    ...
    # Знаменник конверсії: sold + lost
    "лід (не купив)",
}
_LEAD_STATUSES = {
    # НЕ продаж, НЕ у конверсії — активні/непридатні ліди в обробці
...
def _categorize_status(s):
    ...
    # Безпечні fallback'и (рідкісні варіації, не в основному списку)
...
def fetch_salesdrive(date_str: str, uh_1c_data: dict = None) -> dict:
    ...
    try:
        ...
        # ── Категоризація статусів (узгоджено з sales_kpi.py, еталон 19.05.2026) ──
        # Замовлення (продажі) — 12 статусів без refused
        ORDER_STATUSES = {
            "створена ттн",
            "їде до клієнта",
            "прибув у відділення",
        ...
        # Відмови (НЕ зараховуються в "order", але разом з order формують sold)
        REFUSED_STATUSES = {
            "відмова (відправлено)",
            "відмова (не відправлено)",
        }
        # "Лід (не купив)" — окрема категорія "lost": знаменник конверсії
        LOST_LEAD_STATUSES = {
            "лід (не купив)",
        }
        # Ліди (НЕ у конверсії — активні/непридатні)
        LEAD_STATUSES = {
            "новий",
            "недодзвон",
            "автовідповідач",
        ...
        # Спам / технічні / рекламація (повністю виключаються)
        SPAM_STATUSES = {
            "спам на согласование",
            "рекламный спам",
            "спам",
            "спам дубль",
            "видалений",
...
def aggregate_month_crm(target_month: str, day_limit: int = None) -> dict:
    ...
    try:
        ...
        # ── Категоризація статусів (місяць, узгоджено з sales_kpi.py) ──
        # Еталон власника 19.05.2026 — 12 статусів "order" + 2 "refused" = 14 продажів
        ORDER_STATUSES = {
            "створена ттн",
            "їде до клієнта",
            "прибув у відділення",
        ...
        # Відмови (НЕ "order", але разом з ним = sold)
        REFUSED_STATUSES = {
            "відмова (відправлено)",
            "відмова (не відправлено)",
        }
        # "Лід (не купив)" — категорія "lost": знаменник конверсії
        LOST_LEAD_STATUSES = {
            "лід (не купив)",
        }
        # Ліди в обробці (НЕ у конверсії)
        LEAD_STATUSES = {
            "новий",
            "недодзвон",
            "автовідповідач",
