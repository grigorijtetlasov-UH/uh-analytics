...
# ── Статуси (фінальний еталон — узгоджено з власником, 19.05.2026) ──
#
# ORDER_STATUSES + REFUSED_STATUSES = 14 статусів які рахуються як ПРОДАЖ (sold):
#   1. Створена ТТН          8. Потрібне уточнення/перезвон
#   2. Їде до клієнта        9. Помилка менеджера
#   3. Прибув у відділення  10. Контроль оператора
#   4. Переадресація        11. Відправлено
#   5. Закінчився термін…   12. Отримано
#   6. В виробництві        13. Відмова (відправлено)
#   7. В черзі на відпр…    14. Відмова (не відправлено)
#
# LOST_LEAD_STATUSES = знаменник конверсії разом із sold.
#   Конверсія = sold / (sold + lost), як на дашборді продавців.
#
# LEAD_STATUSES / SPAM_STATUSES — НЕ враховуються у конверсії.
ORDER_STATUSES = {
    "створена ттн", "їде до клієнта", "прибув у відділення", "переадресація",
    "закінчився термін зберігання", "в виробництві", "в черзі на відправлення",
    "потрібне уточнення/перезвон", "помилка менеджера", "контроль оператора",
    "відправлено", "отримано",
}
REFUSED_STATUSES = {
    "відмова (відправлено)", "відмова (не відправлено)",
}
LOST_LEAD_STATUSES = {
    # Втрачені ліди (заявки які не дійшли до замовлення) — у знаменнику конверсії
    "лід (не купив)",
}
LEAD_STATUSES = {
    # НЕ продаж, НЕ у конверсії — це активні/непридатні ліди в обробці
    "новий", "недодзвон", "автовідповідач", "повторне звернення",
    "питання по замовленню", "в обробці", "відвідає шоу-рум",
    "контроль оплати", "лід чати",
}
SPAM_STATUSES = {
    "спам на согласование", "рекламный спам", "спам", "спам дубль",
    "видалений", "дубль", "рекламація",
}


def _categorize_status(s) -> str:
    """Категоризує статус у точній відповідності до еталона власника.

    Повертає: 'order' | 'refused' | 'lost' | 'lead' | 'spam' | 'other'

    ВАЖЛИВО: точні множини мають пріоритет. Fallback по підрядках лишається
    тільки для маловідомих варіацій (рідкісні переклади з російської),
    але вже НЕ ловить нічого з основного списку — щоб уникнути дрейфу логіки.
    """
    ...
    # ── Безпечні fallback'и (для рідкісних варіацій, які не в основному списку) ──
    if "спам" in sl:        return "spam"
    if "не купив" in sl:    return "lost"
    # Решта — поза еталоном (наприклад "Рекламація", "Виправити дані", "Повернення")
    return "other"
